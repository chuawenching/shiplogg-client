...
log_ship(
  ...
  actor:        "codex",                                        # always, inside Codex
  external_url: "https://example.com/changelog/0.3"             # optional, where to see it
)
```

`actor` is the identity of the agent making the call. In Codex it is
always `codex`. Never send `human`: a human does not call MCP tools.
...
Other agents use their own name: `claude_code`, `grok_build`, `grok_bot`,
`openclaw`, `hermes`, `cursor`, `other_agent`.
